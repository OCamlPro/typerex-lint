...
let test_progs = [
  ...
  "let x = 1 in x", [ "letBinding", "tralala";
                      "replaceInsideLet", "let x = 1 in y"];
  "1, 2, 3, 4", [ "tuples", "foo"];
  "if List.length mylist = 0 then foo else bar",
  [ "listCompare", "match mylist with | [] -> foo | _ -> bar" ];
  "function | foo -> true | bar -> true",
  [ "function", "function | foo -> true | bar -> false"];
  "match x with foo -> true | bar -> false", [ "match", "x = foo" ];
  "match x with Some y -> true | None -> false",
  [ "matchPattern", "Option.is_some x" ]
]
  ...
  List.fold_left (fun accu patch ->
    List.map (fun (name, expected) ->
        if (name = Patch.get_name patch) then
          let patched_ast, matches =
            Patch.apply patch (Ast_element.Expression parsed_ast)
          in
