...
let path_to_value list =
  StringValue (Filepath.colonpath_to_string list)
  *)
...
let save opfile =
  ...
  let old_file =
    ...
    if Sys.file_exists old_old_file then Sys.remove old_old_file;
  ...
  try
    ...
    Hashtbl.clear once_values_rev;
    file_of_string temp_file (Buffer.contents oc);
...
let info_of_option prefix o =
  ...
  | [name] ->
	...
	M.option_value = string_of_option_value o !!o;
      }
  ...
  List.map
    (fun oi ->
       ...
       Printf.sprintf "<string> : \t%s (current: %s)"
         (string_of_option oi.M.option_short_help) oi.M.option_value)
